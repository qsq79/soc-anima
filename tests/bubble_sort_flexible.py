...
class BubbleSortStep(Scene):
    def __init__(self, original_array, step_num, **kwargs):
    ...
    def construct(self):
        ...
        # 创建标题
        title = Text(f"冒泡排序第{self.step_num + 1}次排序演示", font_size=48, color=WHITE)
        title.to_edge(UP)
        ...
        original_label = Text(f"第{self.step_num}次排序后的数组", font_size=24, color=GRAY).next_to(original_row, UP)
...
if __name__ == "__main__":
    ...
    num_steps = len(original_array) - 1

    current_array = original_array
    for step in range(num_steps):
        scene = BubbleSortStep(current_array, step)
        scene.render()
        current_array = scene.array
